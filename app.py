...
class PDF_Instance :
    ...
    def _load_parse_outputs(self, file_name: str) -> tuple[Optional[str], Optional[Any]]:
        """Load markdown and middle-json artifacts generated by do_parse."""
        output_dir = os.path.join(self.output_dir, file_name)
        md_path = os.path.join(output_dir, f"{file_name}.md")
        middle_json_path = os.path.join(output_dir, f"{file_name}_middle.json")
        ...
        return md_raw, json_raw

    def _normalize_parse_result(self, parse_result: Any, file_name: str) -> tuple[Optional[str], Optional[Any]]:
        """Support both return-style and write-only versions of do_parse."""
        if isinstance(parse_result, tuple) and len(parse_result) == 2:
            return parse_result
        if isinstance(parse_result, dict):
            return parse_result.get("md_raw"), parse_result.get("json_raw")
        return self._load_parse_outputs(file_name)
    ...
    def process_pdf(self, pdf_bytes: bytes, file_name: str = "input.pdf") -> Any:
        ...
        return self._normalize_parse_result(parse_result, file_name)
...
def pdf_process():
    ...
    if json_raw is None:
        return jsonify({'error': 'PDF parsed, but no JSON output was produced'}), 500
    ...
    if args.app:
        ...
        if args.benchmark:
            ...
            for full_path in pdf_files:
                ...
                pdf_instance.process_pdf(pdf_raw, file_name=os.path.basename(full_path))
    else :
        if args.benchmark:
