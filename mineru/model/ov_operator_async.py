...
class OV_Operator(object):
    ...
    def __base_call__(self, input_tensors):
...
class OnnxSessProcessor(OV_Operator):
    def __init__(self, model, name, core=None, postprocess=None):
        super().__init__(model, name, core, postprocess) 
...
# class TSRUnetProcessor(OV_Operator):
...
class YoloV8OVProcessor(OV_Operator):
    ...
    def __call__(self, input_tensors):
        nsize = super().__base_call__(input_tensors)
...
class PaddleTextDetector(OV_Operator):
    ...
    def __call__(self, images) :   
        # print(f"PaddleTextDetector input batch size: {len(images)}")
        nsize=len(images)
        if self.request :
            self.det_res.sync_clean()
            for i, image in enumerate(images):
...
class TextDetector(OV_Operator):
    ...
    def __call__(self, images) :
        # print(f"TextDetector input batch size: {len(images)}")
        nsize=len(images)
        if self.request :
            self.det_res.sync_clean()
            for i, image in enumerate(images):
...
class TextRecognizerOV(OV_Operator):
    ...
    def __call__(self, norm_img_batch_list) :
        # print(f"TextRecognizerOV input batch size: {len(norm_img_batch_list)}")
        nsize=len(norm_img_batch_list)
